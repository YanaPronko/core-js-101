...
function findFirstSingleChar(str) {
  return [...str].find((item, ind, arr) => arr.indexOf(item) === arr.lastIndexOf(item)) || null;
}
...
function reverseInteger(num) {
  return +(`${num}`).split('').map(Number).reverse().join('');
}
...
function evaluateTicTacToePosition(position) {
  ...
  for (let i = 0; i < 3; i += 1) {
    // Check cols
    if (checkEqual(position[0][i], position[1][i], position[2][i])) {
      return position[0][i];
    }
    // Check rows
    if (checkEqual(position[i][0], position[i][1], position[i][2])) {
      return position[i][0];
    }
  }
  if (checkEqual(position[0][0], position[1][1], position[2][2])) {
    return position[0][0];
  }
  if (checkEqual(position[0][2], position[1][1], position[2][0])) {
    return position[0][2];
  }
  return undefined;
}
